    ...
    <div class="col-lg-12">
      <div class="card m-b-20">
        <div class="card-body">
          ...
          <form action="{{ route('division-manage.update', $division->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <div class="form-group">
              <label for="name_division">Tên đơn vị</label>
              <input type="text" class="form-control" name="name_division" value="{{ $division->name_division }}" id="name_division" required
                placeholder="Nhập tên đơn vị" />
            </div>
            <div class="form-row">
              <div class="form-group col-md-6">
                <label class="mr-sm-2" for="kind_division">Loại đơn vị</label>
                <input type="text" class="form-control" name="kind_division" id="kind_division" value="{{ $division->kind_division }}" required
                  placeholder="Nhập loại đơn vị" />
              </div>

              <div class="form-group col-md-6">
                <label for="division_level">Cấp đơn vị</label>
                <input type="number" class="form-control" name="division_level" id="division_level" value="{{ $division->division_level }}" required
