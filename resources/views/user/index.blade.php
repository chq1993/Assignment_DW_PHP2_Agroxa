                  ...
                  <table id="tech-companies-1" class="table  table-striped">
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td> {{ $user->phone }}</td>
                        <td> {{ $user->password }}</td>
                        <td><a href="{{ route('user.show', $user->id) }}"><img src="{{ asset($user->image) }}"
                              style="max-width:30%; " /></a></td>
                        <td id="td{{ $user->id }}">
                          @if ($user->status === 1)
                          <a onclick="changeStatus({{ $user->id }})" class="fa fa-thumbs-up"
                            style="color: green; font-size: 24px;" />
                          @else
                          <a onclick="changeStatus({{ $user->id }}) " class="fa fa-thumbs-down"
                        ...
                        <td>
                          <form action="{{ route('user.destroy', $user->id) }}" method="post">
                            ...
                            <button class="btn btn-danger" type="submit"
                              onclick="return confirm('Bạn chắc chắn muốn xóa bản ghi này?')">Xóa</button>
                          </form>
                        </td>
                      </tr>
                      @endforeach
